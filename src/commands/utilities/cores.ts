...
import { colorsPanel, findColorRoles, missingColorRoleNames } from '../../services/PanelService.js'
...
export default class extends Commands {
    constructor(client: CustomClient) {
        super(client, {
            name: 'cores',
            description: 'Publica o painel de escolha de cores.',
    ...
    async executeInteraction(_client: CustomClient, interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        const missing = missingColorRoleNames(interaction.guild)
        if (missing.length > 0) {
            await interaction.reply({
                content: `Crie estes cargos antes de publicar o painel: ${missing.map((name) => `\`${name}\``).join(', ')}.`,
        ...
        await channel.send({ flags: MessageFlags.IsComponentsV2, components: [colorsPanel(findColorRoles(interaction.guild))] })
        await interaction.editReply({ content: `Painel de cores publicado em ${channel}.` })
    }
}
